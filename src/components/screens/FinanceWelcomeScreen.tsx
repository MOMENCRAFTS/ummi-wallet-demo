      ...
      >
        ...
        <div className="card glass flourish">
          <h3 style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <QueueScrollIcon size={18} /> {isAr ? 'ما شاركته الوالدة معنا' : "What Mother told us"}
          </h3>
        ...
        <div className="action-buttons">
          <button className="btn btn-primary btn-lg glow-mint" onClick={() => navigate('finance-chat')}>
            <ChatBubbleLeafIcon size={20} /> {isAr ? 'بدء الإعداد الموجّه بالذكاء الاصطناعي' : 'Start AI-Guided Setup'}
          </button>
          <button className="btn btn-outline" onClick={() => navigate('finance-chat')}>
            <DocumentLeafIcon size={18} /> {isAr ? 'الإعداد يدويًا' : 'Set Up Manually'}
          </button>
        </div>
      </motion.div>
    </div>
